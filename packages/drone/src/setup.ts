...
const LOCAL_LICENSE_FILE = path.join(__dirname, '../drone-license.json')
...
async function main() {
  ...
  try {
    ...
    console.log('3. Rename it to "drone-license.json" and place it in the "license" folder.')
    console.log('-------------------------------------------------------------\n')
    ...
    const licenseData = JSON.parse(fs.readFileSync(LOCAL_LICENSE_FILE, 'utf-8'))
